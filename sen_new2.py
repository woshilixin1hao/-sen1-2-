# ###
# 修改了 SAR 分支的卷积块，增加了一个选择性核融合模块（SKFusion），
# 并在下采样阶段和中间层都引入了这个融合模块来强化 SAR 特征与光学特征的融合。
# 下采样阶段的修改主要是通过 zip 并行遍历光学主干、SAR 大核分支、SAR 小核分支和融合模块，使得每一层的处理都能同时考虑到这三者的特征。
# 中间层的修改则是在原有的基础上增加了一个 SKFusion 模块，将中间层提取的 SAR 特征进行融合后再注入到光学主干中，进一步增强了模型对 SAR 特征的利用能力。
# 这些修改旨在提升模型在处理 SAR 图像时的性能，使其能够更好地捕捉 SAR 图像中的结构信息，从而提高生成图像的质量。
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="./logs_new2")
    p.add_argument("--model_dir", type=str, default="./models_new2")
    p.add_argument("--sample_dir", type=str, default="./samples_new2")
    p.add_argument("--resume", type=str, default="./models_new2/ckpt_99_380000.pt", help="Path to checkpoint to resume from")
