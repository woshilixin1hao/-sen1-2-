# ###
#在sen_new2的基础上将原本对sar的简单卷积替代为了残差块，来增强对SAR图像特征的提取能力。同时引入了选择性核融合模块（Selective Kernel Fusion），在每个层级对大核和小核分支的特征进行自适应加权融合，以更好地结合两种卷积提取的细节和结构信息。
...
class Sar_conv_large(nn.Module):
    ...
    def forward(self, x):
        return self.conv(x)
#用提取特征的残差块，分别处理 SAR 图像的细节特征（小卷积块）和结构特征（大卷积块），并通过残差连接来增强特征传递和梯度流动。
class Sar_large_block(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()         
        self.block1 = Sar_conv_large(in_channels, out_channels)
        self.block2 = Sar_conv_large(out_channels, out_channels)
    ...
    def forward(self, x):
        ...
        out = self.block1(x)
        out = self.block2(out)
        return (out + shortcut)/1.414  #除以 sqrt(2) 来保持特征的幅度稳定，防止过大或过小，促进训练的稳定性和收敛速度。
    
class Sar_small_block(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()         
        self.block1 = Sar_conv_small(in_channels, out_channels)
        self.block2 = Sar_conv_small(out_channels, out_channels)
        self.res_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1) if in_channels != out_channels else nn.Identity()
...
class SKFusion(nn.Module):
    def __init__(self, channels, reduction=4):
        ...
        self.gamma = nn.Parameter(torch.ones(1) * 0.1)  # 可学习的融合权重，初始值较小，训练过程中会逐渐调整到合适的范围
    ...
    def forward(self, cond_s, cond_l):
        ...
        return cond_fused*self.gamma  #乘以可学习的融合权重 gamma，来控制融合的强度，初始值较小，训练过程中会逐渐调整到合适的范围
# ---------------------------
# 条件 U-Net
# ---------------------------
class Unet(nn.Module):
    def __init__(
        ...
        # ==================== 中间层构建 ====================
        mid_dim = dims[-1]
        self.mid_sarl = Sar_large_block(mid_dim, mid_dim)
        self.mid_sars = Sar_small_block(mid_dim, mid_dim)
...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            loss_avg = loss_total / args.accumulation_steps
            loss_avg.backward()
            global_step += 1

            if global_step % 10 == 0:
                writer.add_scalar("train/loss_total", loss_total.item(), global_step)
                writer.add_scalar("train/loss_simple", loss_simple.item(), global_step)
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="./logs_new6")
    p.add_argument("--model_dir", type=str, default="./models_new6")
    p.add_argument("--sample_dir", type=str, default="./samples_new6")
    p.add_argument("--resume", type=str, default="", help="Path to checkpoint to resume from")
    p.add_argument("--cuda", type=str, default="cuda:2")
