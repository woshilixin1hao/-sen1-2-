# 采用了双分支的设计，主干网络专注于光学图像的特征提取和生成，而两个辅助分支则专门处理 SAR 图像的特征提取。
# 每个分支都包含了两个卷积块和一个下采样层，确保在不同分辨率下都能有效地提取 SAR 特征。
# 通过在每个下采样阶段将 SAR 特征直接注入到主干网络中，实现了两者的深度融合。此外，在中间层也引入了 SAR 特征的融合，进一步增强了模型对 SAR 图像结构信息的捕捉能力。
# 这种设计使得模型能够更好地利用 SAR 图像中的独特信息，从而提升生成图像的质量和细节表现。
import os
import math
import time
import argparse
...
class Unet(nn.Module):
    ...
    def forward(self, x, time, cond):
        ...
        for (opt_blocks, sarl_blocks, sars_blocks) in zip(self.downs, self.sarl, self.sars):
            # 解包网络层
            ...
            x = block1(x, t)
            x = x + (cond_l + cond_s)/2
            # 【注入核心】：因为维度完全一致，直接相加即可，融合结构信息！
            h.append(x)

            cond_l = l_conv2(cond_l)
            cond_s = s_conv2(cond_s)
            x = block2(x, t)
            x = x + (cond_l + cond_s)/2  # 直接加，强化融合
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="./logs_new1")
    p.add_argument("--model_dir", type=str, default="./models_new1")
    p.add_argument("--sample_dir", type=str, default="./samples_new1")
    p.add_argument("--resume", type=str, default="./models_new1/ckpt_89_342000.pt", help="Path to checkpoint to resume training from")
