...
class Unet(nn.Module):
    def __init__(
        ...
        self.channels = channels #输入通道
        self.cond_channels = cond_channels #条件通道（SAR是单通道）
        input_channels = channels + cond_channels #总通道：optical(3) + sar(1) = 4

        #初始卷积
        init_dim = default(init_dim, dim) #获取输入第一层通道安全值
        self.init_conv = nn.Conv2d(input_channels, init_dim, 1, padding=0) # 这是网络的入口。它不改变图片大小，只是把输入图片的通道数变成网络的初始通道数

        #计算每层通道数 (dims)
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)] #最高为128*4=512，这是个列表，从init_dim出发
        in_out = list(zip(dims[:-1], dims[1:])) #ji

        block_klass = partial(ResnetBlock, groups=resnet_block_groups) #偏函数功能，给 ResnetBlock 类预先“填好”一个参数（groups），生成一个新的类构造器 block_klass，以便后面在写代码时更省事。

        # 时间编码
        time_dim = dim * 4 #需要比resblock更大的维度才行
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (num_resolutions - 1) #返回值为布尔类型

            self.downs.append(
                nn.ModuleList(
                    [
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim),
                        block_klass(dim_in, dim_in, time_emb_dim=time_dim), #先两次resnet连接
                        Residual(PreNorm(dim_in, LinearAttention(dim_in))), #然后进行群体标准化然后进行加注意力层操作
                        Downsample(dim_in, dim_out)if not is_last else nn.Conv2d(dim_in, dim_out, 3, padding=1),
                    ] #判断is_last 是不是最后一层，关键一步。图片高宽减半（H/2, W/2），通道数变大（变成 dim_out）
                )
            )

        mid_dim = dims[-1]#返回列表最后一个值
        #中间层，进行两次256通道数resblock卷积
        self.mid_block1 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)
        self.mid_attn = Residual(PreNorm(mid_dim, Attention(mid_dim)))
        self.mid_block2 = block_klass(mid_dim, mid_dim, time_emb_dim=time_dim)

        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            is_last = ind == (len(in_out) - 1) #判断ind等不等于5

            self.ups.append(
                nn.ModuleList(
                    [
                        block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim), #resblock （输入通道数变大了，既有原来的也有现在的）
                        block_klass(dim_out + dim_in, dim_out, time_emb_dim=time_dim),
                        Residual(PreNorm(dim_out, LinearAttention(dim_out))),
                        Upsample(dim_out, dim_in)  if not is_last else nn.Conv2d(dim_out, dim_in, 3, padding=1),#将尺寸变大倍
                    ]
                )
            )
    ...
    def forward(self, x, time, cond=None):
        # cond 是条件图像（SAR），需要和 x（optical）拼接
        if cond is not None:
            x = torch.cat((x, cond), dim=1)
        else:
            # 如果没有条件，用零填充
            b, c, h, w = x.shape
            cond_zeros = torch.zeros(b, self.cond_channels, h, w, device=x.device)
            x = torch.cat((x, cond_zeros), dim=1)
...
def diffusion_loss(model, scheduler, x0, cond, device):
    """预测噪声的 MSE。"""
    b = x0.shape[0]
    t = torch.randint(0, scheduler.T, (b,), device=device).long()
    xt, noise = scheduler.q_sample(x0, t)
    noise_pred = model(xt, t,cond)#model中传入的就是Xt,也就是加噪声之后的图
...
def train(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            loss = diffusion_loss(model, scheduler, x, y, device)
            loss_avg = loss / args.accumulation_steps
            loss_avg.backward()
            global_step += 1
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="./logs_new0")
    p.add_argument("--model_dir", type=str, default="./models_new0")
    p.add_argument("--sample_dir", type=str, default="./samples_new0")
    p.add_argument("--resume", type=str, default="./models_new0/ckpt_99_380000.pt", help="Path to checkpoint to resume from")
    p.add_argument("--cuda", type=str, default="cuda:0")
    ...
    p.add_argument("--save_interval", type=int, default=3800)  # 约每个 epoch 保存1次 (sen1-2)
