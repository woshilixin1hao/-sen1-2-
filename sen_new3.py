# 修改学习率，采用了带预热的余弦衰减策略。首先计算总的优化步数（total_opt_steps），
# 然后设置预热步数为总步数的 5%。在训练循环中，每当执行一次 optimizer.step() 后，就调用 lr_scheduler.step() 来更新学习率。
# 此外，还将当前学习率写入 TensorBoard 日志和进度条显示，方便监控学习率的变化情况。这些修改有助于模型在训练初期快速适应，并在后期稳定收敛，从而提升训练效果。
import os
import math
import time
import argparse
        ...
        for batch in pbar:
            ...
            if global_step % args.accumulation_steps == 0:
                ...
                if global_step % (args.accumulation_steps * 10) == 0: # 没必要每步都存，降个频
                    ...
                    pbar.set_postfix({"loss": float(loss.item()), "lr": f"{current_lr:.2e}"})
...
def parse_args():
    ...
    p.add_argument("--log_dir", type=str, default="./logs_new3")
    p.add_argument("--model_dir", type=str, default="./models_new3")
    p.add_argument("--sample_dir", type=str, default="./samples_new3")
    p.add_argument("--resume", type=str, default="./models_new3/ckpt_99_380000.pt", help="Path to checkpoint to resume training from")
    p.add_argument("--cuda", type=str, default="cuda:4")
